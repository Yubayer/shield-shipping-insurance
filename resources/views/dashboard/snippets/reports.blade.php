@php
    ...
    // dump($start_date, $end_date, $diff_date);
    ...
    function generateGraphData1($startTime, $endTime, $all_orders)
    {
        ...
        $protectionRevenue = number_format($protectOrders__->sum('protection_price'), 2);
    ...
    if ($dateType == 'day') {
        ...
        $startDate = $start_date->copy();
        $endDate = $end_date->copy();
        while ($startDate <= $endDate) {
            $totalOrdersLabel[] = $startDate->format('Y-m-d');
            $chartData = generateGraphData1($startDate, $startDate->copy()->addDay(), $all_orders);
            $totalOrdersData[] = $chartData['total_orders'];
            $protectedOrderData[] = $chartData['protected_orders'];
            $optInRate[] = $chartData['rate'];
            $protectionRevenue[] = $chartData['revenue'];
        ...
        //generate date
        $startOfMonth = $start_date->copy();
        $endOfMonth = $end_date->copy();
        ...
        // }
        $startOfWeek = $start_date->copy();
        $endOfWeek = $end_date->copy();
        while ($startOfWeek <= $endOfWeek) {
            $totalOrdersLabel[] = $startOfWeek->format('Y-m-d');
            $startOfWeek->addDay(10);
        }
        foreach ($totalOrdersLabel as $label) {
            $chartData = generateGraphData1(now()->parse($label), now()->parse($label)->addDay(10), $all_orders);
            $totalOrdersData[] = $chartData['total_orders'];
            $protectedOrderData[] = $chartData['protected_orders'];
            $optInRate[] = $chartData['rate'];
            $protectionRevenue[] = $chartData['revenue'];
